  ...
  if(isset($_POST['AddBooking'])){
     ...
     if($conn->connect_error)
          ...
          { 
            $b = $_GET['bn'];
            echo$b;
            $query="select count(busno) as seatAvailable from booking where busno=$b";
            $query_run=mysqli_query($conn,$query);
            $resultd=mysqli_fetch_array($query_run,MYSQLI_ASSOC);
            
            
            if ($resultd['seatAvailable']>=10)
              ...
              {echo "seats Available= ".$query;
             
            
         
            $stmt=$conn->prepare("INSERT INTO booking(passenger_name,telephone,email,boarding_place,Your_destination,busno) VALUES(?,?,?,?,?,?)");

            $stmt->bind_param("sssssi",$passenger,$tel,$email,$board_place,$desti,$b);
